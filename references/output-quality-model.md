...
Label text must match the owning note content. Treat a repeated default label strip such as `策划注意 / 程序注意 / UI注意 / 动效注意` as a failure when the note lines are not actually discipline-specific.
...
Every generated 4.0 major module must show at least one visible `AI待确认` note in the right-side explanation area. If no real question appears during analysis, re-read the source for missing interaction decisions, feature-loop gaps, design risks, or player-facing state communication gaps before generating.
...
- Does the flow order match the real player path, including confirmation before result, reward/result before return, and branch ownership? Reject diagrams that preserve a template sample path over the analyzed path.
- Does every important subsection/interface state from the source appear as a complete 4.0 row with title, wireframe, and right-side notes? Reject modules that show only the first interface while leaving later functions as headings or hidden analysis.
