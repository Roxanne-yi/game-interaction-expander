...
const REQUIRED_RENDERER_VERSION = "manifest-renderer-v0.9";
...
const REQUIRED_RENDERER_SCRIPT_HASH = "render-brief-board:scaffold-llm-content:2026-06-24";
...
async function main() {
  ...
  if (!record) issue(issues, "fail", "RENDER_RECORD_MISSING", "Board has no manifest renderRecord. Use the official scaffold renderer; do not validate old template-lock boards.", board);
  if (record && record.rendererVersion !== REQUIRED_RENDERER_VERSION) {
    issue(issues, "fail", "UNOFFICIAL_RENDERER", `Board renderRecord.rendererVersion must be ${REQUIRED_RENDERER_VERSION}; found ${record.rendererVersion || "unknown"}. Do not validate manifest-inspired or hand-drawn boards as official output.`, board);
  }
  if (record && record.scriptHash !== REQUIRED_RENDERER_SCRIPT_HASH) {
    issue(issues, "fail", "RENDERER_SCRIPT_HASH_MISMATCH", `Board renderRecord.scriptHash must be ${REQUIRED_RENDERER_SCRIPT_HASH}; found ${record.scriptHash || "missing"}. Do not trust copied scripts with only the version string changed.`, board);
  }
  if (record && !/official-template-scaffold/.test(record.renderMode || "")) {
    issue(issues, "fail", "RENDER_MODE_MISMATCH", "Official renderer must use template scaffold mode so LLM owns interaction content and code owns template boundaries.", board);
  }
  if (record && record.semanticReviewRequired !== true) {
    issue(issues, "fail", "SEMANTIC_REVIEW_NOT_REQUIRED", "Structural validation is not enough. The board must require LLM dual-view semantic review before delivery.", board);
  }
  if (record && (!Array.isArray(record.protectedSnapshots) || record.protectedSnapshots.length === 0)) {
    issue(issues, "fail", "PROTECTED_SNAPSHOTS_MISSING", "Official renderer must store protected template snapshots for header/footer/template fidelity checks.", board);
  }
  const boardManifest = parseJson(board.getSharedPluginData ? board.getSharedPluginData(manifest.namespace, "manifest") : "");
  if (!boardManifest) {
    issue(issues, "fail", "MANIFEST_SHARED_DATA_MISSING", "Official renderer must write manifest shared data on the board.", board);
  } else if (!boardManifest.source_template || boardManifest.source_template.node_id !== manifest.source_template.node_id) {
    issue(issues, "fail", "MANIFEST_SHARED_DATA_MISMATCH", "Board manifest shared data does not match the validator manifest source template.", board);
  }
  ...
  await validateFlows(board, template, manifest, record, issues);
  await validateFeatureDetails(board, template, manifest, record, issues);
  await validateFooter(board, template, manifest, issues);
  validateForbiddenFallbackNodes(board, issues);
...
async function validateVersionRows(board, template, manifest, record, issues) {
  ...
  const rows = [];
  walk(container, (node) => {
    if (node.type === "INSTANCE" && node.name === "子项" && isActuallyVisible(node)) rows.push(node);
  });
...
async function validateFlows(board, template, manifest, record, issues) {
  ...
  if (diagram) {
    ...
    walk(diagram, (node) => {
      ...
      if (node.name === "flow screen node" && isContainer(node)) screenGroups.push(node);
      if (node.name === "flow edge connector") connectorNodes.push(node);
      if (node.name === "flow edge label" && node.type === "TEXT") conditionLabels.push(node);
    });
    ...
    if (connectorNodes.length === 0) issue(issues, "fail", "FLOW_CONNECTORS_MISSING", "Flow diagram must contain connectors.", diagram);
    if (conditionLabels.length === 0 && !/触发|条件|点击|确认|分支|判断/.test(textContent(diagram))) issue(issues, "fail", "FLOW_CONDITION_LABEL_MISSING", "Flow diagram must contain condition labels.", diagram);
  }
  validateRenderedFlowCounts(board, record, issues);
}

function validateRenderedFlowCounts(board, record, issues) {
  if (!record) return;
  const expectedSteps = Number(record.expectedFlowStepCount || 0);
  const expectedEdges = Number(record.expectedFlowEdgeCount || 0);
  const renderedSteps = [];
  const renderedEdges = [];
  walk(board, (node) => {
    if (!isActuallyVisible(node)) return;
    if (node.name === "flow screen node") renderedSteps.push(node);
    if (node.name === "flow edge connector") renderedEdges.push(node);
  });
  if (expectedSteps > 0 && renderedSteps.length < expectedSteps) {
    issue(issues, "fail", "FLOW_STEP_COUNT_MISMATCH", `Flow renderer dropped steps: expected ${expectedSteps}, rendered ${renderedSteps.length}.`, board);
  }
  if (expectedEdges > 0 && renderedEdges.length < expectedEdges) {
    issue(issues, "fail", "FLOW_EDGE_COUNT_MISMATCH", `Flow renderer dropped edges: expected ${expectedEdges}, rendered ${renderedEdges.length}. Do not rely on the template's fixed path.`, board);
  }
}
...
async function validateFeatureDetails(board, template, manifest, record, issues) {
  ...
  if (aiPending.length === 0) issue(issues, "fail", "AI_PENDING_MISSING", "4.0 must include visible AI待确认 notes. They are a core output, not optional decoration.", board);
  validateRenderedFeatureInterfaceCount(board, record, issues);
  validateActualWireframes(board, record, issues);
  validateAiPendingPerMajorModule(board, rightSideTexts, issues);
  validateDefaultLabelCopy(rightSideTexts, issues);
...
function validateRenderedFeatureInterfaceCount(board, record, issues) {
  if (!record) return;
  const expected = Number(record.expectedFeatureInterfaceCount || 0);
  if (expected <= 0) return;
  const frames = [];
  walk(board, (node) => {
    if (isActuallyVisible(node) && String(node.name || "").startsWith("llm wireframe slot ")) frames.push(node);
  });
  if (frames.length < expected) {
    issue(issues, "fail", "FEATURE_INTERFACE_COUNT_MISMATCH", `4.0 renderer dropped interface explanations: expected ${expected}, rendered ${frames.length}.`, board);
  }
}

function validateActualWireframes(board, record, issues) {
  const expected = Number((record && (record.expectedWireframeCount || record.expectedFeatureInterfaceCount)) || 0);
  const frames = [];
  walk(board, (node) => {
    if (isActuallyVisible(node) && String(node.name || "").startsWith("llm wireframe slot ")) frames.push(node);
  });
  if (expected > 0 && frames.length < expected) {
    issue(issues, "fail", "WIREFRAME_SLOT_COUNT_MISMATCH", `Expected ${expected} LLM wireframe slots, found ${frames.length}.`, board);
    return;
  }
  for (const frame of frames) {
    ...
    walk(frame, (node) => {
      if (!isActuallyVisible(node)) return;
      if (node !== frame) visibleNodeCount += 1;
      if (node.type === "TEXT") visibleTexts.push(node);
    });
    const placeholder = visibleTexts.find((node) => /^(原型|Prototype)$/i.test(String(node.characters || "").trim()));
    if (placeholder) {
      issue(issues, "fail", "WIREFRAME_PLACEHOLDER_REMAINS", "4.0 left frame still shows the template placeholder instead of a rendered wireframe.", placeholder);
    }
    if (visibleNodeCount < 8) {
      issue(issues, "fail", "WIREFRAME_NOT_RENDERED", `4.0 left frame has too little LLM-drawn interface content (${visibleNodeCount} nodes). Draw player-visible UI structure inside the slot.`, frame);
    }
    if (visibleTexts.length > 0 && visibleTexts.length >= visibleNodeCount - 2) {
      issue(issues, "fail", "WIREFRAME_TEXT_ONLY", "4.0 wireframe slot looks text-only. It must contain drawn interface structure, controls, and states.", frame);
    }
    const content = visibleTexts.map((node) => node.characters || "").join("\n");
    if (/规则|机制|后台|说明[:：]/.test(content) && visibleNodeCount < 16) {
      issue(issues, "fail", "WIREFRAME_LOOKS_LIKE_RULE_CARD", "4.0 wireframe appears to be a rule card or mechanism summary instead of a player-visible interface.", frame);
    }
  }
}

function validateAiPendingPerMajorModule(board, rightSideTexts, issues) {
  const section4 = allTexts(board)
    .filter(isActuallyVisible)
    .find((node) => String(node.characters || "").trim() === "4.0");
  const section4Y = section4 ? rel(section4, board).y : 0;
  const titles = allTexts(board)
    .filter(isActuallyVisible)
    .map((node) => ({ node, text: String(node.characters || "").trim(), rb: rel(node, board) }))
    .filter((item) => item.rb && item.rb.y > section4Y && /^4\.\d+/.test(item.text))
    .sort((a, b) => a.rb.y - b.rb.y);
  for (let i = 0; i < titles.length; i += 1) {
    const top = titles[i].rb.y;
    const bottom = i + 1 < titles.length ? titles[i + 1].rb.y : Number.POSITIVE_INFINITY;
    const hasAi = rightSideTexts.some((node) => {
      const rb = rel(node, board);
      return rb && rb.y >= top && rb.y < bottom && /AI待确认/.test(node.characters || "");
    });
    if (!hasAi) {
      issue(issues, "fail", "AI_PENDING_MISSING_IN_MODULE", `Major module ${titles[i].text} must include a visible AI待确认 note.`, titles[i].node);
    }
  }
}

function validateDefaultLabelCopy(rightSideTexts, issues) {
  const labels = rightSideTexts
    .map((node) => ({ node, text: String(node.characters || "").trim(), rb: rel(node, node.parent || node) }))
    .filter((item) => /^(策划注意|程序注意|UI注意|动效注意)$/.test(item.text));
  const hasAi = rightSideTexts.some((node) => /AI待确认/.test(node.characters || ""));
  const defaultNames = new Set(labels.map((item) => item.text));
  if (!hasAi && defaultNames.has("策划注意") && defaultNames.has("程序注意") && defaultNames.has("UI注意")) {
    issue(issues, "fail", "RIGHT_LABELS_LOOK_TEMPLATE_DEFAULT", "Right-side labels look like copied template defaults and no AI待确认 is visible. Choose semantic labels from the actual interface notes.", labels[0] && labels[0].node);
  }
}
...
function validateForbiddenFallbackNodes(board, issues) {
  const forbiddenNames = new Set(["generated missing text slot", "flow screen generated title"]);
  const found = [];
  walk(board, (node) => {
    if (isActuallyVisible(node) && forbiddenNames.has(node.name)) found.push(node);
  });
  if (found.length > 0) {
    issue(issues, "fail", "FORBIDDEN_FALLBACK_NODE_PRESENT", `Forbidden fallback nodes are visible (${found.length}). Missing manifest slots/prototypes must stop generation instead of creating fallback text.`, found[0]);
  }
}
